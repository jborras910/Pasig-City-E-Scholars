...
$sql = "SELECT * FROM users_table WHERE  active_status='active' AND user_type !='scholar' AND user_type !='Main_Admin'";
...
if ($result->num_rows > 0) {
    ...
    while($row = $result->fetch_assoc()) {
        ...
        $data[] = array(
        $counter, 
        date("F j, Y, g:i a", strtotime($row['date_created'])),
...
$file_name = 'ADMINS ' . (date('Y') - 1) . '-' . date('Y') . '.pdf';
...
$pdf->Cell(0, 10, 'PASIG CITY SCHOLARSHIP PROGRAM '.'ADMINS ' . (date('Y') - 1) . '-' . date('Y'), 0, 1, 'C');
...
$columnNames = array('No', 'Date Created', 'Name', 'Email Address');
