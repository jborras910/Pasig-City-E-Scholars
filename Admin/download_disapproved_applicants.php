...
$sql = "SELECT * FROM applicants_table WHERE isApproved='disapproved'";
...
if ($result->num_rows > 0) {
    ...
    while($row = $result->fetch_assoc()) {
        ...
        date("F j, Y, g:i a", strtotime($row['disapproved_date'])),
...
$file_name = 'Disapproved Applicants ' . (date('Y') - 1) . '-' . date('Y') . '.pdf';
...
$pdf->Cell(0, 10, 'PASIG CITY SCHOLARSHIP PROGRAM '.'DISAPPROVED APPLICANTS ' . (date('Y') - 1) . '-' . date('Y'), 0, 1, 'C');
...
$columnNames = array('No', 'Date Submitted', 'Date Disapproved', 'Name','Location','Email');
