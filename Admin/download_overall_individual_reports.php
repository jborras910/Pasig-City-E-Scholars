...
$admin_name = $_SESSION['auth_user']['first_name']." ".$_SESSION['auth_user']['Middle_Name']." ".$_SESSION['auth_user']['last_name'];
// Retrieve data from the database
$sql = "SELECT * FROM applicants_table WHERE assists_by='$admin_email'";
...
if ($result->num_rows > 0) {
    ...
    while($row = $result->fetch_assoc()) {
        $status = ($row['isApproved'] == 'true') ? 'Approved' : 'Disapproved';
...
$file_name = 'Overall Applicants ' . (date('Y') - 1) . '-' . date('Y') . '.pdf';
...
$pdf->Cell(0, 10, 'PASIG CITY SCHOLARSHIP PROGRAM '.'OVERALL APPLICANTS ' . (date('Y') - 1) . '-' . date('Y'), 0, 1, 'C');
...
$pdf->Cell(0, 1,"Evaluator Name: ".$admin_name, 0, 1, 'L'); 
$currentDateTime = date("F j, Y, g:i a", strtotime(date('Y-m-d H:i:s')));


$pdf->Cell(10, 10,"Date: ".$currentDateTime, 0, 1, 'L'); 
...
$columnNames = array('No', 'Date Submitted', 'Status', 'Name','Location','Email');
